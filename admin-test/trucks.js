  ...
  function render() {
    $('trucksBody').innerHTML = truckRecords.map((truck) => `
      <tr data-enhanced-truck="${esc(truck.id)}">
        ...
        <td>
          <span class="truck-compliance ${expiryClass(truck.quarterly_inspection)}">Quarterly: ${esc(formatDate(truck.quarterly_inspection))}</span>
          <span class="truck-compliance ${expiryClass(truck.annual_inspection)}">Annual: ${esc(formatDate(truck.annual_inspection))}</span>
          <span class="truck-compliance ${expiryClass(truck.insurance_expiration)}">Insurance: ${esc(formatDate(truck.insurance_expiration))}</span>
        </td>
