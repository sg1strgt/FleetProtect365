(() => {
  "use strict";
  // Reuse the Driver location experience with the authenticated Admin company context.
    ...
    dialog.innerHTML = `
      <div class="fedex-driver-shell">
        <header class="fedex-driver-header">
          <div><div class="fedex-wordmark"><span>Fed</span><b>Ex</b> Locations</div><small>Wade Freight Systems</small></div>
  ...
  async function loadLocations() {
    locations = await window.FP365_ADMIN_FEDEX_LOCATIONS();
  ...
  async function openLocations() {
    ...
    try {
      ...
      byId("fedexDriverBody").innerHTML = `<section class="fedex-driver-message"><h2>Locations unavailable</h2><p>${esc(error.message)}</p><button type="button" data-fedex-return>Return to Admin</button></section>`;
    ...
    byId("fedexDriverResult").innerHTML = `
      <article class="fedex-location-card">
        ...
        <section class="fedex-driver-summary">
          ...
          <div class="fedex-driver-actions"><a href="${mapsUrl}" target="_blank" rel="noopener">⌖ Open in Maps</a>${selected.phone ? `<a href="tel:${String(selected.phone).replace(/\D/g,"")}">☎ Call</a>` : ""}<button type="button" data-fedex-return>⌂ Return to Admin</button></div>
        </section>
  ...
  document.addEventListener("click", event => {
    if (event.target.closest("[data-open-admin-fedex-locations]")) openLocations();
  });
  window.FP365_OPEN_ADMIN_FEDEX_LOCATIONS = openLocations;
})();
