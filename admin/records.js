(() => {
  ...
  function renderTimeOff(){const rows=filteredRecords('timeoff');table('timeoff',['Driver','From','To','Comments','Action'],rows.map(r=>`<tr><td>${esc(r.driver_name)}</td><td>${fmtDate(r.date_from)}</td><td>${fmtDate(r.date_to)}</td><td>${esc(r.comments||'—')}</td><td>${actionButtons('timeoff',r.id)}</td></tr>`));}
