(() => {
  ...
  let company, profile, drivers=[], trucks=[], recipients=[], state={dispatch:[],callout:[],timeoff:[],daily:[],dispatchlog:[],mileage:[]};
  ...
  const definitions = {
    ...
    dispatchlog:{table:'dispatch_log',date:'dispatch_date',title:'Dispatch Log'},
  ...
  function shell(){
    ...
    hub.innerHTML=`<div class="records-intro"><h2>Reports and Records</h2><p>Maintain attendance, time-off, daily assignments, and route-mile records.</p></div>
      ...
      ${card('daily','Daily Dispatch Record','Daily driver, run, dispatch, and truck assignments.','Add Record','<button type="button" data-copy-all-daily>Copy All</button><button type="button" data-save-dispatch-log>Save to Dispatch Log</button><button type="button" data-refresh-daily>Clear Today’s Records</button>')}
      ${card('dispatchlog','Dispatch Log','Editable dispatch database with up to three legs per daily assignment.','Add Row','<button type="button" data-export-dispatch-log="pdf">Export PDF</button><button type="button" data-export-dispatch-log="csv">Export CSV</button>')}
    ...
    hub.querySelector('[data-save-dispatch-log]').onclick=saveDailyToDispatchLog;
    hub.querySelector('[data-save-all-dispatch-log]').onclick=saveAllDispatchLog;
    hub.querySelectorAll('[data-export-dispatch-log]').forEach(button=>button.onclick=()=>openDispatchLogExport(button.dataset.exportDispatchLog));
  ...
  async function loadAll(){
    try{
      ...
      const queries=Object.entries(definitions).map(([key,d])=>client.from(d.table).select('*').eq('company_id',company.id).gte(d.date,['mileage','dispatchlog'].includes(key)?'1900-01-01':cutoffIso).order(d.date,{ascending:true}));
  ...
  function renderAll(){renderCallouts();renderTimeOff();renderDaily();renderDispatchLog();renderMileage();bindRows();bindDispatchLogRows();}
  ...
  function renderDispatchLog(){const rows=dispatchLogRows();table('dispatchlog',['Date','Driver','Run','Dispatch Time','Truck Number','Notes','Leg / Action'],rows.map(r=>`<tr data-dispatch-log-row="${r.id}"><td><input data-log-field="dispatch_date" type="date" value="${r.dispatch_date}" aria-label="Dispatch date"></td><td><select data-log-field="driver_profile_id" aria-label="Driver">${driverOptions(r.driver_profile_id)}</select></td><td><input data-log-field="run" value="${esc(r.run)}" aria-label="Run"></td><td><input data-log-field="dispatch_time" type="time" value="${r.dispatch_time?String(r.dispatch_time).slice(0,5):''}" aria-label="Dispatch time"></td><td><select data-log-field="truck_number" aria-label="Truck number">${truckOptions(r.truck_number)}</select></td><td><textarea data-log-field="notes" maxlength="500" rows="2" aria-label="Notes">${esc(r.notes||'')}</textarea></td><td><div class="dispatch-log-actions"><b>Leg ${r.leg_number}</b><button type="button" data-add-log-leg="${r.id}" ${r.leg_number>=3?'disabled':''}>Add Leg</button><button type="button" class="danger" data-delete-log-row="${r.id}">Delete</button></div></td></tr>`));}
  ...
  function dispatchLogRows(){const groups=new Map();state.dispatchlog.forEach(row=>{const key=row.source_daily_id?`daily:${row.source_daily_id}`:`log:${row.parent_log_id||row.id}`;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(row);});return [...groups.values()].map(rows=>{rows.sort((a,b)=>Number(a.leg_number)-Number(b.leg_number));return{rows,base:rows.find(row=>Number(row.leg_number)===1)||rows[0]};}).sort((a,b)=>b.base.dispatch_date.localeCompare(a.base.dispatch_date)||String(b.base.created_at).localeCompare(String(a.base.created_at))||String(a.base.driver_name).localeCompare(String(b.base.driver_name))).flatMap(group=>group.rows);}
  function dispatchLogRecordFromRow(id){const element=document.querySelector(`[data-dispatch-log-row="${id}"]`),value=name=>element.querySelector(`[data-log-field="${name}"]`).value,row=state.dispatchlog.find(item=>item.id===id),driver=drivers.find(item=>item.id===value('driver_profile_id'));if(!driver)throw Error('Select a driver.');if(!value('dispatch_date')||!value('run').trim()||!value('truck_number'))throw Error('Complete Date, Driver, Run, and Truck Number.');return{dispatch_date:value('dispatch_date'),driver_profile_id:driver.id,driver_name:driver.full_name||driver.display_name,run:value('run').trim(),dispatch_time:value('dispatch_time')||null,truck_number:value('truck_number'),notes:value('notes').trim()||null,updated_by:profile.id,updated_at:new Date().toISOString(),leg_number:row.leg_number};}
  function dispatchLogGroup(row){const key=row.source_daily_id?`daily:${row.source_daily_id}`:`log:${row.parent_log_id||row.id}`;return state.dispatchlog.filter(item=>(item.source_daily_id?`daily:${item.source_daily_id}`:`log:${item.parent_log_id||item.id}`)===key).sort((a,b)=>Number(a.leg_number)-Number(b.leg_number));}
  function bindDispatchLogRows(){document.querySelectorAll('[data-add-log-leg]').forEach(button=>button.onclick=()=>addDispatchLogLeg(button.dataset.addLogLeg));document.querySelectorAll('[data-delete-log-row]').forEach(button=>button.onclick=()=>deleteDispatchLogLeg(button.dataset.deleteLogRow));}
  function deleteDispatchLogLeg(id){const row=state.dispatchlog.find(item=>item.id===id),group=row?dispatchLogGroup(row):[];if(group.length<=1)return removeRecord('dispatchlog',id);const available=group.map(item=>item.leg_number).join(', '),answer=prompt(`This assignment has multiple legs. Which leg should be deleted? Available legs: ${available}`,String(row.leg_number));if(answer===null)return;const leg=Number(answer),target=group.find(item=>Number(item.leg_number)===leg);if(!target)return alert(`Enter one of the available leg numbers: ${available}.`);removeRecord('dispatchlog',target.id);}
  async function saveAllDispatchLog(){try{const records=[...document.querySelectorAll('[data-dispatch-log-row]')].map(element=>({id:element.dataset.dispatchLogRow,company_id:company.id,...dispatchLogRecordFromRow(element.dataset.dispatchLogRow)}));if(!records.length)throw Error('There are no Dispatch Log rows to save.');$('dispatchlogSummary').innerHTML='<p>Saving all Dispatch Log rows…</p>';const {error}=await client.from('dispatch_log').upsert(records,{onConflict:'id'});if(error)throw error;await loadAll();$('dispatchlogSummary').innerHTML=`<p><b>All ${records.length} Dispatch Log row${records.length===1?'':'s'} saved.</b></p>`;}catch(error){$('dispatchlogSummary').innerHTML=`<p class="records-empty">${esc(error.message)}</p>`;}}
  async function addDispatchLogLeg(id){try{const source=state.dispatchlog.find(row=>row.id===id);if(!source)throw Error('Dispatch Log row was not found.');const groupKey=source.source_daily_id?`daily:${source.source_daily_id}`:`log:${source.parent_log_id||source.id}`,related=state.dispatchlog.filter(row=>(row.source_daily_id?`daily:${row.source_daily_id}`:`log:${row.parent_log_id||row.id}`)===groupKey),nextLeg=Math.max(0,...related.map(row=>Number(row.leg_number)||1))+1;if(nextLeg>3)throw Error('This assignment already has three legs.');const current=dispatchLogRecordFromRow(id),record={...current,dispatch_time:null,company_id:company.id,source_daily_id:source.source_daily_id||null,parent_log_id:source.parent_log_id||source.id,leg_number:nextLeg,created_by:profile.id};const {error}=await client.from('dispatch_log').insert(record);if(error)throw error;await loadAll();$('dispatchlogSummary').innerHTML=`<p><b>Leg ${nextLeg} added directly beneath ${esc(source.driver_name)} with Dispatch Time blank.</b> Add a time only if needed, then use Save All.</p>`;}catch(error){$('dispatchlogSummary').innerHTML=`<p class="records-empty">${esc(error.message)}</p>`;}}
  function selectedDailyRows(){if(!state.daily.length)return{date:'',rows:[]};const dates=[...new Set(state.daily.map(row=>row.dispatch_date))].sort().reverse(),date=dates.includes(today())?today():dates[0];return{date,rows:state.daily.filter(row=>row.dispatch_date===date)};}
  async function saveDailyToDispatchLog(){const {date,rows}=selectedDailyRows();if(!rows.length)return void($('dailySummary').innerHTML='<p class="records-empty">No Daily Dispatch records are available to save.</p>');$('dailySummary').innerHTML='<p>Saving Daily Dispatch records to the Dispatch Log…</p>';const stamp=new Date().toISOString(),records=rows.map(row=>({company_id:company.id,source_daily_id:row.id,leg_number:1,dispatch_date:row.dispatch_date,driver_profile_id:row.driver_profile_id,driver_name:row.driver_name,run:row.run,dispatch_time:row.dispatch_time||null,truck_number:row.truck_number,notes:row.notes||null,created_by:profile.id,updated_by:profile.id,updated_at:stamp}));const {error}=await client.from('dispatch_log').upsert(records,{onConflict:'company_id,source_daily_id,leg_number'});if(error)return void($('dailySummary').innerHTML=`<p class="records-empty">${esc(error.message)}</p>`);await loadAll();$('dailySummary').innerHTML=`<p><b>${rows.length} record${rows.length===1?'':'s'} for ${fmtDate(date)} saved to the Dispatch Log.</b></p>`;$('dispatchlogSummary').innerHTML='<p><b>Dispatch Log updated from Daily Dispatch.</b></p>';}
  function localIso(date){return `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;}
  function dispatchExportPeriod(period){const now=new Date(),current=today();if(period==='today')return{from:current,to:current,label:`Today — ${fmtDate(current)}`,file:`today_${current}`};if(period==='weekly'){const from=new Date(now.getFullYear(),now.getMonth(),now.getDate()),offset=(from.getDay()+1)%7;from.setDate(from.getDate()-offset);const to=new Date(from);to.setDate(to.getDate()+6);const fromIso=localIso(from),toIso=localIso(to);return{from:fromIso,to:toIso,label:`This Week — ${fmtDate(fromIso)} through ${fmtDate(toIso)}`,file:`week_${fromIso}_to_${toIso}`};}const from=new Date(now.getFullYear(),now.getMonth(),1),to=new Date(now.getFullYear(),now.getMonth()+1,0),fromIso=localIso(from),toIso=localIso(to);return{from:fromIso,to:toIso,label:`This Month — ${fmtDate(fromIso)} through ${fmtDate(toIso)}`,file:`month_${fromIso.slice(0,7)}`};}
  function ensureDispatchExportDialog(){if($('dispatchExportDialog'))return;const dialog=document.createElement('dialog');dialog.id='dispatchExportDialog';dialog.innerHTML='<form><h2 id="dispatchExportTitle">Export Dispatch Log</h2><p>Choose the date period to include. Weeks run Saturday through Friday.</p><div class="actions"><button type="button" data-dispatch-export-period="today">Today</button><button type="button" data-dispatch-export-period="weekly">This Week</button><button type="button" data-dispatch-export-period="monthly">This Month</button><button type="button" data-dispatch-export-cancel>Cancel</button></div></form>';document.body.appendChild(dialog);dialog.querySelector('[data-dispatch-export-cancel]').onclick=()=>dialog.close();dialog.querySelectorAll('[data-dispatch-export-period]').forEach(button=>button.onclick=()=>{const format=dialog.dataset.format;dialog.close();exportDispatchLog(format,button.dataset.dispatchExportPeriod);});}
  function openDispatchLogExport(format){ensureDispatchExportDialog();const dialog=$('dispatchExportDialog');dialog.dataset.format=format;$('dispatchExportTitle').textContent=`Export Dispatch Log as ${format.toUpperCase()}`;dialog.showModal();}
  function exportDispatchLog(format,period){const range=dispatchExportPeriod(period),rows=dispatchLogRows().filter(row=>row.dispatch_date>=range.from&&row.dispatch_date<=range.to);if(!rows.length)return void($('dispatchlogSummary').innerHTML=`<p class="records-empty">There are no Dispatch Log rows for ${esc(range.label)}.</p>`);const headers=['Date','Driver','Run','Dispatch Time','Truck Number','Notes','Leg'],body=rows.map(row=>[fmtDate(row.dispatch_date),row.driver_name,row.run,fmtTime(row.dispatch_time),row.truck_number,row.notes||'',row.leg_number]);if(format==='csv'){const quote=value=>`"${String(value??'').replace(/"/g,'""')}"`,csv=[headers,...body].map(row=>row.map(quote).join(',')).join('\r\n'),url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'})),link=document.createElement('a');link.href=url;link.download=`FleetProtect365_Dispatch_Log_${range.file}.csv`;link.click();URL.revokeObjectURL(url);}else{const doc=new window.jspdf.jsPDF({orientation:'landscape'});doc.setFontSize(18);doc.setTextColor(22,38,63);doc.text('Fleet Protect 365 — Dispatch Log',14,18);doc.setFontSize(10);doc.setTextColor(70);doc.text(range.label,14,24);doc.autoTable({startY:29,head:[headers],body,styles:{fontSize:8,cellPadding:2,overflow:'linebreak'},columnStyles:{5:{cellWidth:70}},headStyles:{fillColor:[22,38,63]}});doc.save(`FleetProtect365_Dispatch_Log_${range.file}.pdf`);}$('dispatchlogSummary').innerHTML=`<p><b>${rows.length} Dispatch Log row${rows.length===1?'':'s'} for ${esc(range.label)} exported as ${format.toUpperCase()}.</b></p>`;}

  function resetEditorFields(){if($('recordFields'))$('recordFields').replaceChildren();if($('recordType'))$('recordType').value='';if($('recordId'))$('recordId').value='';if($('recordFormMsg'))$('recordFormMsg').textContent='';}
  function closeEditor(){const dialog=$('recordDialog');if(dialog?.open)dialog.close();resetEditorFields();}
  function ensureDialog(){if($('recordDialog'))return;const d=document.createElement('dialog');d.id='recordDialog';d.className='record-dialog';d.innerHTML='<form id="recordForm"><h2 id="recordDialogTitle"></h2><input id="recordType" type="hidden"><input id="recordId" type="hidden"><div id="recordFields"></div><p id="recordFormMsg"></p><div class="actions"><button id="recordReset" type="button">Refresh / Clear</button><button id="recordCancel" type="button">Cancel</button><button class="primary" type="submit">Save Record</button></div></form>';document.body.appendChild(d);d.onclose=resetEditorFields;$('recordCancel').onclick=closeEditor;$('recordReset').onclick=()=>openEditor($('recordType').value,null,true);$('recordForm').onsubmit=saveRecord;}
  async function copyDailyText(){
    const {date:selectedDate,rows}=selectedDailyRows();if(!rows.length)return void($('dailySummary').innerHTML='<p class="records-empty">No Daily Dispatch records are available to copy.</p>');
  ...
  function timeField(name,label,value='',required=true,allowNA=false){const hasValue=Boolean(value),[h24='',m='']=String(value||'').split(':'),n=Number(h24),period=hasValue?(n>=12?'PM':'AM'):'',h=hasValue?(n%12||12):'',minute=hasValue?String(Number(m||0)).padStart(2,'0'):'',requiredAttr=required?'required':'';return `<label>${label}${required?' *':' (optional)'}<span class="record-time${allowNA?' allow-na':''}" data-time="${name}"><input inputmode="numeric" maxlength="2" value="${h}" aria-label="Hour" ${requiredAttr}><b>:</b><input inputmode="numeric" maxlength="2" value="${minute}" aria-label="Minute" ${requiredAttr}><select aria-label="AM or PM" ${requiredAttr}><option value="" ${!period?'selected':''}>AM/PM</option><option ${period==='AM'?'selected':''}>AM</option><option ${period==='PM'?'selected':''}>PM</option></select>${allowNA?`<button type="button" data-clear-time="${name}">NA</button>`:''}</span></label>`;}
  ...
  async function saveRecord(event){
    event.preventDefault();const type=$('recordType').value,id=$('recordId').value;try{let record={company_id:company.id,updated_by:profile.id,updated_at:new Date().toISOString()};
      ...
      if(type==='dispatchlog'){const d=selectedDriver();Object.assign(record,{dispatch_date:$('recordDate').value,driver_profile_id:d.id,driver_name:d.name,run:$('recordRun').value.trim(),dispatch_time:readOptionalTime('dispatchlog'),truck_number:$('recordTruck').value.trim(),notes:$('recordDailyNotes').value.trim()||null,leg_number:1});}
      ...
      if(!id)record.created_by=profile.id;const query=id?client.from(definitions[type].table).update(record).eq('id',id).eq('company_id',company.id):client.from(definitions[type].table).insert(record);const {error}=await query;if(error)throw error;closeEditor();await loadAll();
