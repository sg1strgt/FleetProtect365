(() => {
  ...
  function openEditor(type,row=null,reset=false){
    ...
    if(type==='daily')$('recordDailyNotes').oninput=()=>{$('dailyNotesCount').textContent=`${$('recordDailyNotes').value.length} / 500`;};
  ...
  async function saveRecord(event){
    event.preventDefault();const type=$('recordType').value,id=$('recordId').value;try{let record={company_id:company.id,updated_by:profile.id,updated_at:new Date().toISOString()};
      ...
      if(type==='daily'){const d=selectedDriver();Object.assign(record,{dispatch_date:$('recordDate').value,driver_profile_id:d.id,driver_name:d.name,run:$('recordRun').value.trim(),dispatch_time:readTime('daily'),truck_number:$('recordTruck').value.trim(),notes:$('recordDailyNotes').value.trim()||null});}
