(() => {
  ...
  async function invoke(body) {
    ...
    if (sessionError || !sessionData?.session?.access_token) {
      throw new Error('Your admin session has expired. Log out, log back in, and try again.');
    }
    const response = await fetch(`${config.supabaseUrl}/functions/v1/manage-user`, {
      method: 'POST',
      headers: {
    ...
    try {
      ...
      throw new Error(`Credential service returned HTTP ${response.status} without an explanation.`);
    }
    if (!response.ok || !data?.ok) {
      throw new Error(data?.error || data?.message || `Credential service returned HTTP ${response.status}.`);
    }
    return data;
  }
  ...
  $('saveDriver').addEventListener('click', async (event) => {
    ...
    if ($('driverRole').value === 'driver') {
      requiredIds.push('driverDlNumber', 'driverDlState', 'driverDlExpires', 'driverMedExpires');
    }
