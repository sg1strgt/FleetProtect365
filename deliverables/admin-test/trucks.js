(() => {
  ...
  function render() {
    ...
    $('trucksBody').innerHTML = visible.map((truck) => `
      <tr data-enhanced-truck="${esc(truck.id)}">
        ...
        <td><div class="truck-action"><button type="button" data-edit-truck-profile="${esc(truck.id)}">Edit</button>${truck.status !== 'retired' ? `<button type="button" class="danger" data-remove-truck="${esc(truck.id)}">Remove</button>` : ''}</div></td>
      </tr>`).join('');
  ...
  $('trucksBody').addEventListener('click', (event) => {
    ...
    const reason = prompt(`Why is truck ${truck?.truck_number || ''} being removed?`);
    ...
    (async () => {
      ...
      await loadTrucks();
      $('trucksMsg').textContent = `Truck ${truck.truck_number} was retired.`;
    })();
  });
