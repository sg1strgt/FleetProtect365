(() => {
  ...
  function render() {
    ...
    const visible = records.filter((record) => !search || [
      ...
      record.dolly_number, record.trailer_2_number,
    ...
    $('dispatchBody').innerHTML = visible.map((record) => `
      <tr>
        <td><div class="dispatch-line">${esc(formatDate(record.dispatch_date))}</div><div class="dispatch-line dispatch-muted">Run ${esc(record.run_number)}</div></td>
        <td><div class="dispatch-line">${esc(record.driver_name)}</div><div class="dispatch-line dispatch-muted">Employee ID: ${esc(record.employee_id)}</div></td>
        <td>${esc(record.location_from)} → ${esc(record.location_to)}</td>
        <td><div class="dispatch-line">Truck: ${esc(record.truck_number)}</div><div class="dispatch-line">Trailer 1: ${esc(record.trailer_number)}</div>${record.dolly_number ? `<div class="dispatch-line">Dolly: ${esc(record.dolly_number)}</div>` : ''}${record.trailer_2_number ? `<div class="dispatch-line">Trailer 2: ${esc(record.trailer_2_number)}</div>` : ''}</td>
        <td><div class="dispatch-line">Scheduled: ${esc(formatTime(record.scheduled_dispatch))}</div><div class="dispatch-line">Actual: ${esc(formatTime(record.actual_dispatch))}</div></td>
  ...
  function updateRun() {
    ...
    if (!id && !$('dispatchRun').value) {
      $('dispatchRun').value = nextRunNumber($('dispatchDriver').value, $('dispatchDate').value);
    }
  ...
  function openDialog(id = '') {
    ...
    if (record) {
      ...
      $('dispatchDolly').value = record.dolly_number || '';
      $('dispatchTrailer2').value = record.trailer_2_number || '';
  ...
  async function saveRecord() {
    ...
    const record = {
      ...
      run_number: Number($('dispatchRun').value),
      ...
      dolly_number: $('dispatchDolly').value.trim().toUpperCase() || null,
      trailer_2_number: $('dispatchTrailer2').value.trim().toUpperCase() || null,
  ...
  function refreshSuggestedRun() {
    if (!$('dispatchId').value) {
      $('dispatchRun').value = nextRunNumber($('dispatchDriver').value, $('dispatchDate').value);
    }
    updateRun();
  }
  $('dispatchDriver').onchange = refreshSuggestedRun;
  $('dispatchDate').onchange = refreshSuggestedRun;
  ...
  $('dispatchForm').onsubmit = async (event) => {
    ...
    const requestedRun = Number($('dispatchRun').value);
    const duplicateRun = records.some((record) =>
      record.id !== $('dispatchId').value &&
      record.driver_profile_id === $('dispatchDriver').value &&
      record.dispatch_date === $('dispatchDate').value &&
      Number(record.run_number) === requestedRun);
    if (!Number.isInteger(requestedRun) || requestedRun < 1) {
      $('dispatchFormMsg').textContent = 'Run Number must be a whole number of 1 or greater.';
      return;
    }
    if (duplicateRun) {
      $('dispatchFormMsg').textContent = 'That driver already has this Run Number on the selected date.';
      return;
    }
  ...
  $('exportDispatch').onclick = () => {
    const headers = ['Date', 'Run', 'Driver', 'Employee ID', 'Truck', 'Trailer 1', 'Dolly', 'Trailer 2', 'From', 'To',
    ...
    const rows = records.map((record) => [record.dispatch_date, record.run_number, record.driver_name,
      record.employee_id, record.truck_number, record.trailer_number, record.dolly_number,
      record.trailer_2_number, record.location_from,
